...
import { BotUserInfoDto, User } from '../../models/user';
...
export class BotUserController {
    ...
    private async normalizeSlackAndBotUsers(bots: BotUserInfoDto[], slack: Member[]): Promise<User[]> {
        const users: User[] = [];

        for await (const user of slack) {
            // if slack user is in db, tack db data onto slack user object
            let dbUser = bots.find(botUser => botUser.slack_id === user.id);
            if (!dbUser) {
